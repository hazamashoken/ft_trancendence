...
export class MutedService {
  ...
  async findAllMutedAtChat(
    ...
    return mutedUsers.map(mutedUser => plainToClass(ReturnMutedDto, mutedUser));
  }
  ...
  async deleteMuted(
    ...
    const chat = await this.channelRepository.findOne({
      where: { chatId: chatId },
      relations: ['mutedUsers', 'chatOwner', 'chatAdmins', 'mutedUsers.user'],
    });
    ...
    if (!chat.mutedUsers.find(user => (user.user.id = userId)))
