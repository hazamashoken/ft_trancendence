...
export function collision(state: GameState, delta: number) {
  const halfsquare: number = Pong.SQUARE_SIZE / 2;
  const halfbat: number = (Pong.SQUARE_SIZE * Pong.BAT_SIZE) / 2;
  const seconds: number = delta / 1000;
  let i = 0;

  // move bat and ball
  state.player1.position.y +=
    seconds * Pong.BAT_SPEED * state.player1.direction.y;
  state.player2.position.y +=
    seconds * Pong.BAT_SPEED * state.player2.direction.y;
  ...
  for (i = 0; i < state.multiball.length; i++) {
    state.multiball[i].position.x +=
      seconds * Pong.BALL_SPEED * state.multiball[i].direction.x;
    state.multiball[i].position.y +=
      seconds * Pong.BALL_SPEED * state.multiball[i].direction.y;
  }

  // bat with wall collision
  if (state.player1.position.y < halfbat) state.player1.position.y = halfbat;
  if (state.player1.position.y > 1 - halfbat)
    state.player1.position.y = 1 - halfbat;

  if (state.player2.position.y < halfbat) state.player2.position.y = halfbat;
  ...
  if (
    state.ball.position.x - halfsquare <
      state.player1.position.x + halfsquare &&
    ...
    if (
      ...
      if (state.player1.direction.y != 0) {
        ...
        const mag = Math.sqrt(
          state.ball.direction.x ** 2 + state.ball.direction.y ** 2,
        );
        state.ball.direction.x =
          (state.ball.direction.x / mag) * Pong.BALL_SPEED;
        state.ball.direction.y =
          (state.ball.direction.y / mag) * Pong.BALL_SPEED;
      }
  ...
  if (
    state.ball.position.x + halfsquare >
      state.player2.position.x - halfsquare &&
  ...
  for (i = 0; i < state.multiball.length; i++) {
    if (
      state.multiball[i].position.x - halfsquare <
        state.player1.position.x + halfsquare &&
      ...
      if (
        ...
        state.multiball[i].position.x =
          state.player1.position.x + Pong.SQUARE_SIZE;
        ...
        if (state.player1.direction.y != 0) {
          ...
          state.multiball[i].direction.x =
            (state.multiball[i].direction.x / mag) * Pong.BALL_SPEED;
          state.multiball[i].direction.y =
            (state.multiball[i].direction.y / mag) * Pong.BALL_SPEED;
        }
        if (state.player1.powerup) {
          state.multiball[i].direction.x *= 2;
          state.multiball[i].direction.y *= 2;
    ...
    if (
      ...
      if (
        ...
        state.multiball[i].position.x =
          state.player2.position.x - Pong.SQUARE_SIZE;
        state.multiball[i].direction.x = -state.multiball[i].direction.x;
        // change the direction of the ball if the bat is moving
        if (state.player2.direction.y != 0) {
          // add half the bat speed to the ball
...
export function winner(state: GameState): string {
  const square3: number = Pong.SQUARE_SIZE * 3;
  let i = 0;
